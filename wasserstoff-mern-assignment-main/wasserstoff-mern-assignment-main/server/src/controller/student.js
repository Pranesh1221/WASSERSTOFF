...
const createStudent = async (req, res) => {
  ...
  if (Object.keys(req.body).length == 0)
    return res
      .status(400)
      .send({ status: false, message: "please fill the all fields" });
  ...
  if (!/^[A-Za-z\s]{1,}[\.]{0,1}[A-Za-z\s]{0,}$/.test(name))
    return res
      .status(400)
      .send({ status: false, message: "name should be only letter" });
  if (!topic || topic.trim().length === 0)
    return res
      .status(400)
      .send({ status: false, message: "please fill the topic fields" });
  if (!summary || summary.trim().length === 0)
    return res
      .status(400)
      .send({ status: false, message: "please fill the summary fields" });

  const student = await studentModel.findOne({ name });
  if (student)
    return res
      .status(400)
      .send({ status: false, message: `${name} is already present` });

  req.body.topics = {
    // insert  topics key and set a object in req.body.topics
    topic,
    summary,
  };
